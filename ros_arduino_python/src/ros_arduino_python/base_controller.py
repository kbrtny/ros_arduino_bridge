...
class BaseController:
    def __init__(self, arduino, base_frame):
        ...
        '''
        try: # arduino_simulate wants a copy of base:
            arduino.base_for_simulation(self)
        except:
            pass
        '''
    ...
    def poll(self):
        ...
        if now > self.t_next:
            ...
            if self.enc_left == None:
                dright = 0
                dleft = 0
            elif bool(self.arduino.port):
                dright = (right_enc - self.enc_right) / self.ticks_per_meter
                dleft = (left_enc - self.enc_left) / self.ticks_per_meter
            else: # if no arduino.port, simulate encoders:
                dright = self.v_des_right * dt * self.ticks_per_meter
                dleft = self.v_des_left * dt * self.ticks_per_meter
